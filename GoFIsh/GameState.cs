...
namespace GoFish
{
    ...
    public class GameState
    {
        ...
        {
            //var valuePlural = (valueToAskFor == Values.Шестёрка) ? "Шестёрка" : $"{valueToAskFor}s";
            ...
            if(player.Hand.Count() == 0)                   //Если у игрока нет карт в руке
            {
                ...
                message += $"{Environment.NewLine}{player.Name} раздал все свои карты, и взял {player.Hand.Count()} карт{(player.Hand.Count() > 1 ? "ы" : "у")} из колоды";                        //Добавить сообщение о том что игрок взял карты
            }
        ...
        public string CheckForWinner()
        {
            ...
            if (playerCards > 0) return "";                                                  //Если сумма больше 0, вернуть пустую строку и выйти из метода
